...
def add_scales(width: float, height: float, img: Image.Image) -> Image.Image:
    draw = ImageDraw.Draw(img)
    font = ImageFont.load_default()

    for x in range(100, width - 100, 50):
        draw.text((x, height - 75), str(x - 100), fill="black", font=font)
...
def ft_zoom(path: str) -> Image.Image:
    ...
    #     draw.text((60, y), str(y -100), fill="black", font=font)
    # extended_img = add_scales(new_width, new_height, extended_img)
    return extended_img
